...
public class DefaultGarbageProcessor implements GarbageProcessor {
    private static final String GARBAGE_DOES_NOT_HAVE_DISPOSAL_ANNOTATION = "The passed in garbage does not implement an annotation implementing the Disposable meta-annotation or is not supported by the StrategyHolder.";
    ...
    public ProcessingData processWaste(Waste garbage) {
        ...
        if (disposableAnnotation == null || !this.getStrategyHolder().getDisposalStrategies().containsKey(disposableAnnotation)) {
            throw new IllegalArgumentException(GARBAGE_DOES_NOT_HAVE_DISPOSAL_ANNOTATION);
        }
        currentStrategy = this.getStrategyHolder().getDisposalStrategies()
